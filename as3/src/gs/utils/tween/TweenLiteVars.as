...
package gs.utils.tween {
	...
	dynamic public class TweenLiteVars {
		...
		/**
		 * Works with clone() to copy all the necessary properties. Split apart from clone() to take advantage of inheritence for TweenFilterLiteVars and TweenMaxVars
		 */
		protected function appendCloneVars($vars:Object, $protectedVars:Object):void {
			var props:Array = ["autoAlpha","delay","ease","easeParams","endArray","onStart","onStartParams","onUpdate","onUpdateParams","onComplete","onCompleteParams","overwrite","persist","removeTint","renderOnStart","runBackwards"];
			for (var i:int = props.length - 1; i > -1; i--) {
				$vars[props[i]] = this[props[i]];
			}
			$protectedVars._exposedInternalProps = _exposedInternalProps;
			$protectedVars._frame = _frame;
			$protectedVars._tint = _tint;
			$protectedVars._visible = _visible;
			$protectedVars._volume = _volume;
